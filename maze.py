...
def read_generator(file, sigma, omega, tau):
    ...
    final_output_with_cycles(tau, main_dict, sigma, omega)
...
def main_input():
    while (True):
        inp = raw_input("To create a maze, type: initMaze N K k p M W H G T sigma omega tau\n"
                        "To load a maze, type: loadMaze filename sigma omega tau\n\n")
        sinp = inp.split()

        if sinp[0].lower() == "initMaze".lower():

            N, K, k, p, M, W, H, G, T, sigma, omega, tau = int(sinp[1]), int(sinp[2]), int(sinp[3]), int(sinp[4]), \
                                                           int(sinp[5]), int(sinp[6]), int(sinp[7]), int(sinp[8]), int(
                sinp[9]), \
                                                           int(sinp[10]), float(sinp[11]), int(sinp[12])
            ...
            final_output_with_cycles(tau, new_dict, sigma, omega)
            output()
            break

        elif sinp[0].lower() == "loadMaze".lower():

            fname, sigma, omega, tau = sinp[1], int(sinp[2]), float(sinp[3]), int(sinp[4])
            ...
            if omega > 1:
                print ("Decay cannot be greater than 1. Try again.")
                continue
            x = read_generator(fname, sigma, omega, tau)
...
def print_maze(dict):
    ...
    for key, value in dict.items():
        ...
        #                                                                  "          Number of teleports: " + unicode(
        #     key.pres_teleport) + "   Number of Walls: " + unicode(key.pres_wall) + u"    \n"
        print(stri)
    print("\n\n===================================================================\n\n")
...
#==================================================================================================================#
    ...
    # Print changed rooms
    print ("\nCycle " + unicode(globalClock[0]) + "\n")
    #print_maze(dict)
    for room in changed:
         print_one_room(dict, room)

def final_output_with_cycles(tau, dict, sigma, omega):
...
# initMaze 10 5 3 5 3 2 2 0 2 2 0.5 10
